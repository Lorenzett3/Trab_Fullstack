...
export class PedidoController {
    ...
    listarPorCliente = async (req: Request, res: Response): Promise<void> => {
        const clienteId = req.params.clienteId;
        try {
            const pedidos = await this.service.buscarPorClienteId(clienteId);
            res.json(pedidos);
        } catch (err: any) {
            res.status(err.id || 500).json({ error: err.msg || "Erro ao buscar pedidos por cliente" });
        }
    };
